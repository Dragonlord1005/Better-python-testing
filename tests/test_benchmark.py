...
def test_only_override_skip(testdir):
    ...
    result = testdir.runpytest_subprocess('--benchmark-only',
                                          '--benchmark-skip', test)
    result.stdout.fnmatch_lines([
        "*collected 2 items",
...
def test_max_time_min_rounds(testdir):
    test = testdir.makepyfile(SIMPLE_TEST)
    result = testdir.runpytest_subprocess('--doctest-modules',
...
def test_pep418_timer(testdir):
    ...
    result = testdir.runpytest_subprocess(
        '--benchmark-max-time=0.0000001', '--doctest-modules',
...
def test_compare(testdir):
    test = testdir.makepyfile(SIMPLE_TEST)
    testdir.runpytest_subprocess('--benchmark-max-time=0.0000001',
                                 '--doctest-modules', '--benchmark-autosave',
                                 test)
    result = testdir.runpytest_subprocess('--benchmark-max-time=0.0000001',
                                          '--doctest-modules',
...
def test_save(testdir):
    ...
    json.loads(
        testdir.tmpdir.join('.benchmarks').listdir()[0].join(
            '0001_foobar.json').read())
...
def test_simple(benchmark):
    ...
    def run_verify_pytest(*args):
        testdir.runpytest_subprocess('--benchmark-json=benchmark.json',
                                     '--benchmark-max-time=0.0000001', *args)

        benchmark_json = json.loads(
            testdir.tmpdir.join('benchmark.json').read())
        machine_info = benchmark_json["machine_info"]

        assert sorted(
            i.replace('\\', '/')
            for i in machine_info["conftest_path"]) == sorted([
                "conftest.py",
...
def test_histogram(testdir):
    ...
    assert [f.basename
            for f in testdir.tmpdir.listdir("*.svg", sort=True)] == [
                'foobar.svg',
            ]
...
def test_autosave(testdir):
    ...
    json.loads(
        testdir.tmpdir.join('.benchmarks').listdir()[0].listdir('0001_*.json')
        [0].read())
...
def test_xdist(testdir):
    ...
    test = testdir.makepyfile(SIMPLE_TEST)
    result = testdir.runpytest_subprocess('--doctest-modules', '-n', '1',
                                          '-rw', test)
    result.stderr.fnmatch_lines([
...
def test_disabled_and_cprofile(testdir):
    test = testdir.makepyfile(SIMPLE_TEST)
    result = testdir.runpytest_subprocess('--benchmark-disable',
                                          '--benchmark-cprofile=cumtime', test)
    result.stdout.fnmatch_lines([
        "*==== 2 passed*",
    ])
...
def test_skip(testdir):
    test = testdir.makepyfile(BASIC_TEST)
    result = testdir.runpytest_subprocess('-vv', '--doctest-modules',
                                          '--benchmark-skip', test)
    result.stdout.fnmatch_lines([
        "*collected 5 items",
...
def test_mark_selection(testdir):
    test = testdir.makepyfile(BASIC_TEST)
    result = testdir.runpytest_subprocess('-vv', '--doctest-modules', '-m',
                                          'benchmark', test)
    result.stdout.fnmatch_lines([
        "*collected 5 items*",
...
def test_columns(testdir):
    test = testdir.makepyfile(SIMPLE_TEST)
    result = testdir.runpytest_subprocess(
        '--doctest-modules', '--benchmark-columns=max,iterations,min', test)
    result.stdout.fnmatch_lines([
        "*collected 3 items",
        "test_columns.py ...*",
        "* benchmark: 2 tests *",
